...
uniform sampler2D g_Texture3; // {"default":"util/white","label":"ui_editor_properties_time_offset","mode":"opacitymask"}
...
void main() {
	...
	pulse = v_AudioPulse;
#else
	float flowPhase = texSample2D(g_Texture3, v_TexCoord.zw).r * M_PI_2;
	pulse = smoothstep(g_PulseThresholds.x, g_PulseThresholds.y, sin(g_Time * g_PulseSpeed + g_PulsePhase + flowPhase) * 0.5 + 0.5) * g_PulseAmount;
